...
- **Автозапуск Windows:** Можливість запуску сервера при старті системи через параметр `--autostart on/off`.
...
3. **Параметри автозапуску (Windows):**
   ```bash
   python main.py --autostart on   # Увімкнути автозапуск
   python main.py --autostart off  # Вимкнути автозапуск
   ```
...
### 3. Приклад фонового тестування (Artisan Command)
```php
<?php
...
use App\Services\SoundApiService;

class TestMultiSoundApi extends Command
{
    protected $signature = 'sound:test-all';
    protected $description = 'Тестування всіх доступних SoundAPI інстансів';

    public function handle(SoundApiService $service)
    {
        $instances = config('sound_api.instances');

        foreach ($instances as $name => $config) {
            $this->info("--- Тестування інстансу: {$name} ({$config['url']}) ---");

            try {
                $devices = $service->listDevices($config);

                if (empty($devices)) {
                    $this->warn("Інстанс {$name} доступний, але пристроїв не знайдено.");
                    continue;
                }

                $this->table(['ID', 'Name'], $devices);

                // Тестовий звук на перший пристрій кожного інстансу
                $res = $service->play($config, $devices[0]['id'], "https://soundbible.com/mp3/Stomach%20Growling-SoundBible.com-164767511.mp3");
                
                if (isset($res['status'])) {
                    $this->info("Успішно запущено на {$name}");
                }
            } catch (\Exception $e) {
                $this->error("Інстанс {$name} недоступний: " . $e->getMessage());
            }
        }
    }
}
...
### 🛠 Технічні деталі (Python)
- Стабільність потоків: Використовується `sd.OutputStream` із глобальним сховищем потоків, що запобігає помилкам Garbage Collector на Python 3.13.
- Декодування: Автоматичне налаштування FFmpeg через winget на Windows.
- Безпека: JWT токен (HS256) для захисту доступу до колонок.
- Формати: MP3, OGG, WAV підтримуються через FFmpeg.
- Автозапуск: `--autostart on/off` для Windows Startup.

---

**Автор:** Микола Дмитренко 2026 | SoundAPI 2026
